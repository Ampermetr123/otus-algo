...
def load_as_2_string(f):
    """Выбор из файла двух строк
    1 - первая строка файла
    2 - остальное содержимое"""
    s1 = f.readline()
    s1 = s1.rstrip("\r\n")
    s2 = f.read()
    return (s1, s2)
...
def create_test_cases(
    folder_path, func, inp_loader, out_loader, rng, test_callback
) -> int:
    ...
    for i in rng:
        ...
        if os.path.isfile(inp_file) and os.path.isfile(out_file):
            test_count += 1
            setattr(FolderTestCase, test_name := f"test_{i}", create_test(func))
...
def run_test(
    folder,
    func,
    inp_loader=load_as_strings,
    out_loader=load_as_strings,
    rng=None,
    test_callback=None,
):
    """Setup and run test for _func_ using test data files in _folder_"""

    test_count = create_test_cases(
        os.path.abspath(folder), func, inp_loader, out_loader, rng, test_callback
    )

    if test_count == 0:
        raise UserWarning(
            """%s directory doesn't consist any tests.
            Each test must heave 2 files: test_{x}.in and test_{x}.out"""
            % folder
        )
    else:
